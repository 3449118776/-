...
var MemorySkill = (function () {
  ...
  // ==========================================================================
  // === PART 13：V2 补全方法（道具/势力/滚动摘要/角色状态历史）===============
  // ==========================================================================

  // ---- 记录道具 ----
  MemorySkill.prototype.recordItem = function (name, owner, chapterIdx, description) {
    this._itemLedger.push({
      name: name || '',
      owner: owner || '',
      chapterIdx: chapterIdx != null ? chapterIdx : (this._meta.totalChapters || 0),
      description: description || '',
      timestamp: Date.now()
    });
    this.remember('items', `道具「${name}」归属${owner}，${description || ''}`, { chapter: chapterIdx, tags: ['item'] });
  };

  // ---- 记录势力 ----
  MemorySkill.prototype.recordFaction = function (name, leader, members, chapterIdx, status) {
    if (!this._factionGraph[name]) {
      this._factionGraph[name] = {
        name: name,
        leader: leader || '',
        members: members || [],
        firstSeen: chapterIdx || 0,
        lastSeen: chapterIdx || 0,
        status: status || '活跃',
        history: []
      };
    } else {
      if (chapterIdx != null) this._factionGraph[name].lastSeen = chapterIdx;
      if (leader) this._factionGraph[name].leader = leader;
      if (members && members.length > 0) {
        members.forEach(m => {
          if (this._factionGraph[name].members.indexOf(m) < 0) {
            this._factionGraph[name].members.push(m);
          }
        });
      }
      if (status) this._factionGraph[name].status = status;
    }
    this._factionGraph[name].history.push({
      chapterIdx: chapterIdx || 0,
      event: status || '势力更新',
      timestamp: Date.now()
    });
    this.remember('relationships', `势力「${name}」状态更新：${status || ''}`, { chapter: chapterIdx });
  };

  // ---- 更新滚动摘要 ----
  MemorySkill.prototype.updateRollingSummary = function (chapterIdx, summary) {
    const recentLimit = 10;
    const milestoneLimit = 50;

    // 近期摘要：最近10章
    const recentChapters = this._chapterIndex.slice(-recentLimit);
    this._rollingSummary.recent = recentChapters.map(c => `第${c.chapterIdx + 1}章：${c.summary.substring(0, 60)}`).join('\n');

    // 里程碑：每10章取一章作为里程碑
    const milestones = [];
    for (let i = 0; i < this._chapterIndex.length; i += 10) {
      const ch = this._chapterIndex[i];
      if (ch) milestones.push(`第${ch.chapterIdx + 1}章：${ch.summary.substring(0, 80)}`);
    }
    this._rollingSummary.milestones = milestones.slice(-Math.floor(milestoneLimit / 10)).join('\n');

    // 整卷摘要：全部章节的一句话概括
    this._rollingSummary.eras = this._chapterIndex.map(c => `第${c.chapterIdx + 1}章：${c.summary.substring(0, 40)}`).join(' | ');
  };

  // ---- 记录角色状态历史 ----
  MemorySkill.prototype.recordCharacterState = function (name, chapterIdx, state) {
    if (!this._characterHistory[name]) {
      this._characterHistory[name] = [];
    }
    this._characterHistory[name].push({
      chapterIdx: chapterIdx || 0,
      status: state.status || '',
  ...
  // ---- 获取角色成长轨迹 ----
  MemorySkill.prototype.getCharacterTrajectory = function (name) {
    return this._characterHistory[name] || [];
  };

  // ---- 自动归档（从正文提取信息） ----
  MemorySkill.prototype.autoArchive = function (chapterIdx, fullText, extracted) {
    // 写入章节摘要
    this.recordChapter(chapterIdx, extracted.summary || fullText.substring(0, 200));

    // 更新角色
    if (extracted.characters) {
      extracted.characters.forEach(c => {
        this.recordCharacter(c.name, c.role || '配角', {
          chapter: chapterIdx,
          status: c.status || '',
          location: c.location || '',
          emotion: c.emotion || ''
        });
        this.recordCharacterState(c.name, chapterIdx, c);
      });
    }

    // 记录伏笔
    if (extracted.foreshadows) {
      extracted.foreshadows.forEach(f => {
        this.recordForeshadow(chapterIdx, f.text, f.status || '未解');
      });
    }
    ...
    if (extracted.items) {
      extracted.items.forEach(item => {
        this.recordItem(item.name, item.owner || '', chapterIdx, item.description || '');
      });
    }

    // 记录势力
    if (extracted.factions) {
      extracted.factions.forEach(f => {
        this.recordFaction(f.name, f.leader || '', f.members || [], chapterIdx, f.status || '');
      });
    }

    // 更新时间线
    if (extracted.timeline) {
      this.recordTimeline(chapterIdx, extracted.timeline);
    }

    // 更新滚动摘要
    this.updateRollingSummary(chapterIdx, extracted.summary || '');
  };

  // ---- 增强版矛盾检测（带实体提取） ----
  MemorySkill.prototype._reasoning.detectContradictionsV2 = function (ms) {
    var contradictions = [];
    var coreFacts = ms._anchors.core.map(function (a) { return { text: a.text, chapter: a.chapterIdx }; });
    ...
    function extractEntities(text) {
      ...
      if (matches) {
        matches.forEach(function (m) {
          if (['活着', '死亡', '朋友', '敌人', '拥有', '失去', '强大', '弱小', '知道', '不知道'].indexOf(m) < 0) {
            entities.push(m);
          }
        });
      }
    ...
    for (var i = 0; i < coreFacts.length; i++) {
      var entitiesA = extractEntities(coreFacts[i].text);
      for (var j = i + 1; j < coreFacts.length; j++) {
        var entitiesB = extractEntities(coreFacts[j].text);
        // 检查是否有共同实体
        var commonEntities = entitiesA.filter(function (e) { return entitiesB.indexOf(e) >= 0; });
        ...
        for (var cp = 0; cp < conflictPairs.length; cp++) {
          ...
          if (coreFacts[i].text.indexOf(pair.a) >= 0 && coreFacts[j].text.indexOf(pair.b) >= 0) {
            contradictions.push({
              type: pair.label,
              entity: commonEntities.slice(0, 3).join('、'),
              factA: coreFacts[i].text.substring(0, 50),
              factB: coreFacts[j].text.substring(0, 50),
              severity: 'high'
            });
          }
        }
  ...
  // ---- 增强版检索（Jaccard 相似度） ----
  MemorySkill.prototype.searchByKeywordV2 = function (keyword, maxResults) {
    maxResults = maxResults || 20;
    var keywordTokens = tokenize(keyword);
    var matches = [];

    // 搜索锚点
    Object.keys(this._anchors).forEach(function (key) {
      this._anchors[key].forEach(function (a) {
        var textTokens = tokenize(a.text || '');
        var sim = jaccard(keywordTokens, textTokens);
        if (sim > 0.1 || (a.text || '').indexOf(keyword) >= 0) {
          matches.push({ category: key, text: a.text, chapter: a.chapterIdx, similarity: sim });
        }
      }.bind(this));
    }.bind(this));

    // 搜索章节摘要
    this._chapterIndex.forEach(function (c) {
      var summaryTokens = tokenize(c.summary || '');
      var sim = jaccard(keywordTokens, summaryTokens);
      if (sim > 0.1 || (c.summary || '').indexOf(keyword) >= 0) {
        matches.push({ category: 'chapter', chapterIdx: c.chapterIdx, text: c.summary, similarity: sim });
      }
    });
